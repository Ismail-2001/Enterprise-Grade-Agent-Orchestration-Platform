...
| TLS/mTLS | "requestCert:true" | Traced @grpc/grpc-js 1.14.4 source: `createSsl(caCert, keyCertPairs, true)` correctly sets `requestCert: true` on HTTP/2 server. Prior "blocked" assessment was incorrect. | Confirmed |
...
| 4 | TLS / mTLS | 2 | `packages/shared/src/tls.ts`: `createSsl(caCert, keyCertPairs, true)` → `requestCert: true`. Real CA/server/client certs in `certs/`. `x-service-token` defense-in-depth |
...
12. **mTLS** — `requestCert:true` correctly wired (traced @grpc/grpc-js source)
